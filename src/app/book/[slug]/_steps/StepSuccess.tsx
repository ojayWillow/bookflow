...
import { CheckCircle, Clock, MapPin, Phone, Mail, Globe, Instagram, Facebook } from 'lucide-react'
...
export default function StepSuccess({ business, service, staffMember, date, time, form, bookingRef, emailSent, isPending, dict: t }: Props) {
  ...
  if (isPending) {
    return (
      <div className="text-center py-8">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">{t.pendingTitle}</h2>
        <p className="text-gray-500 mb-1 max-w-sm mx-auto">{t.pendingSubtitle}</p>
        {emailSent && (
          <p className="text-sm text-gray-400 mb-1">
            {t.pendingEmailSent.replace('{{email}}', form.email)}
          </p>
        )}
        <p className="text-sm text-gray-400 mb-8">
          {t.bookingRef}: <span className="font-mono font-bold text-amber-600">{bookingRef}</span>
        </p>

        <div className="bg-white border-2 border-amber-100 rounded-2xl overflow-hidden text-left mb-4">
