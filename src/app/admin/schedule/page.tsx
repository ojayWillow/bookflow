...
export default function SchedulePage() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      ...
      .catch(e => setError(e instanceof Error ? e.message : t.schedule.loadFail))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || t.schedule.saveFail)
      setSaved(true)
      setDirty(false)
      setTimeout(() => setSaved(false), 3000)
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : t.schedule.saveFail)
    } finally {
      setSaving(false)
    }
  }
  ...
  if (!settings) return (
    <div className="p-8">
      <div className="bg-red-50 border border-red-200 text-red-700 rounded-2xl px-6 py-5">
        <p className="font-semibold mb-1">⚠️ {t.schedule.loadFail}</p>
    ...
    <div className="p-6 md:p-8 max-w-2xl">
      ...
      <div className="space-y-6">
        ...
        <button
          ...
          {saved ? t.schedule.saved : saving ? t.schedule.saving : t.schedule.save}
        </button>
      </div>
    </div>
  )
